...
##
## The script is part of the "IoT Kaffeekanne" project, lecture 'Internet of Things' of the Reutlingen University (HHZ), SoSe 2016.
...
QUEUE_MEASUREMENTS_WORKER_INTERVAL = 10 
...
def measurements_worker(q):
    # Worker thread to handle queued measurements.
    # Wait for filled queue
    time.sleep(5)
    while True:
        qsize = q.qsize()
        median = 0
        if qsize > 0:
            ...
            logging.info('Handling measurement: %s' % median)

            try:
                connection = sqlite3.connect("/home/pi/iot/sqlDatenbank/coffeeDB.db")
                cursor = connection.cursor()

                reply = cursor.execute("""INSERT INTO coffeeLevel(timestamp,level) VALUES (%i,%i)"""% (timestamp,median))
...
def main():
    ...
    worker = Thread(target=measurements_worker, args=(queue_measurements,))
